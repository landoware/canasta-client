...
<template>
  <button
    type="button"
    class="relative aspect-769/1065 w-[calc(var(--card-base-width)*var(--card-scale,1))] transition-transform disabled:opacity-50 disabled:cursor-not-allowed enabled:cursor-pointer enabled:hover:-translate-y-1"
  ...
  >
    ...
    <div
      v-if="count === 0"
      class="absolute inset-0 rounded-2xl border-2 border-dashed border-card-white/40"
    />
